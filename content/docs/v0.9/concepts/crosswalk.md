...
InfluxDB is a time series database and there is not a need for full UPDATE or DESTROY functionality.  It is optimized to store high volume, time series data.  This data commonly comes from sources like distributed sensor groups, click data from large websites, lists of financial transactions etc...

What this data has in common is that it is much more useful in the aggregate.  One reading saying that your computer’s CPU is at 12% utilization at 12:38:35 on a Tuesday is useless on it’s own.  It only becomes useful when it is paired with the rest of the series and visualized.  This is where trends over time begin to show, and actionable insight can be drawn from the data.  

Time series also shares the attribute that it is generally written once and never updated.  These observations drive the design and development of InfluxDB.  Updating in InfluxDB is actually an overwrite operation.  This functionality is slow, but allows for batch updating: e.g. a sensor was shown to be consistently wrong by 2° C.  Destroy similarly works only on batches of data: e.g. a whole series or measurement.

The result is that InfluxDB is not a full CRUD database but more like a CR-ud, creating and reading data at performant speeds while having limited update and destroy ability.  For more information on why InfluxDB made these architectural decisions [Paul Dix](https://github.com/pauldix) has an excellent [blog post](https://influxdb.com/blog/2015/06/03/InfluxDB_clustering_design.html) with more explanation.
